...
class CompletedRunWithQueue(CompletedRun):
    queue_map_states: int = field(default=0, hash=True)
...
def profile(exec_name: str, stdin: str, trials=3, input_from_file=True) -> CompletedRun:
    ...
    for i in range(trials):
        ...
        for _line in stdout.split('\n'):
            ...
            if match_queue_map is None:
                match_queue_map = rexp_queue_map_states.match(line)

        if match_rt and match_states and match_queue_map:
            states = int(match_states.group(1))
            run_time = float(match_rt.group(1))
            queue_map_states = int(match_queue_map.group(2))
            ...
            if not match_queue_map: failure_list.append('max queue')
    ...
    return CompletedRunWithQueue(states=states, runtime=avg, sched=sched, queue_map_states=queue_map_states)
